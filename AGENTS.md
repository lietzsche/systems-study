...
- 에이전트가 개념, 실패 시나리오, 관찰 방법과 한 번에 필요한 만큼의 구현 과제를 제시합니다.
- 처음 사용하는 언어 또는 API는 실행 가능한 최소 문법을 먼저 제공합니다.
- 학습자는 핵심 로직을 작성하거나 실행 결과를 예측하고, 실패 결과와 질문을 대화로 전달합니다.
- boilerplate, module README, 학습 기록, 검증과 Git 작업은 에이전트가 담당합니다.

새 API를 외워서 작성하도록 요구하지 않습니다. 직접 작성하는 부분은 현재 시스템 개념을 드러내는 의미 있는 코드로 제한합니다.
...
labs/
  ...
  02-os-file-descriptor/
  03-storage-append-log/
  ...
```
